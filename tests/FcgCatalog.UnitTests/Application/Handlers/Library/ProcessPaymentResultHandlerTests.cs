...
public class ProcessPaymentResultHandlerTests
{
    ...
    public async Task Handle_QuandoApproved_AdicionaJogoAoUsuarioEAprovaOrder()
    {
        ...
        userGameRepo.ExistsAsync(userId, gameId, Arg.Any<CancellationToken>()).Returns(false);

        var handler = new ProcessPaymentResultHandler(orderRepo, userGameRepo, NullLogger<ProcessPaymentResultHandler>.Instance);

        var command = new ProcessPaymentResultCommand(order.Id, ContractStatus.Approved);
        var result = await handler.Handle(command, CancellationToken.None);

        result.IsSuccess.ShouldBeTrue();
        order.Status.ShouldBe(OrderStatus.Approved);
        userGameRepo.Received(1).Add(Arg.Is<UserGame>(u => u.UserId == userId && u.GameId == gameId));
    }
    ...
    public async Task Handle_QuandoRejected_RejeitaOrderENaoAdicionaBiblioteca()
    {
        ...
        var order = new Order(Guid.NewGuid(), Guid.NewGuid(), 100m);
        orderRepo.GetByIdAsync(order.Id, Arg.Any<CancellationToken>()).Returns(order);

        var handler = new ProcessPaymentResultHandler(orderRepo, userGameRepo, NullLogger<ProcessPaymentResultHandler>.Instance);

        var command = new ProcessPaymentResultCommand(order.Id, ContractStatus.Rejected);
    ...
    public async Task Handle_QuandoApproved_E_BibliotecaJaTem_NaoDuplica()
    {
        ...
        var order = new Order(Guid.NewGuid(), Guid.NewGuid(), 100m);
        orderRepo.GetByIdAsync(order.Id, Arg.Any<CancellationToken>()).Returns(order);
        userGameRepo.ExistsAsync(order.UserId, order.GameId, Arg.Any<CancellationToken>()).Returns(true);
